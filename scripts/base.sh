...
# $HOME/distributed-HGNNs/build/torchtest -c "${HOME}/distributed-HGNNs/config/paper_config.yaml" -u ${USER} -t ${TMPDIR}
# mpiexec -np ${SLURM_NTASKS} $HOME/distributed-HGNNs/build/example_2 ${TMPDIR}/data/m_g_ms_gs/DVH.mtx ${TMPDIR}/data/m_g_ms_gs/invDE_HT_DVH.mtx ${TMPDIR}/data/m_g_ms_gs/DVH_times_invDE_HT_DVH.mtx
# mpiexec -np ${SLURM_NTASKS}  $HOME/distributed-HGNNs/build/openBLAS_example
# mpiexec -np ${SLURM_NTASKS}  $HOME/distributed-HGNNs/build/timing
# mpiexec -np ${SLURM_NTASKS}  $HOME/distributed-HGNNs/build/model ${HOME}/distributed-HGNNs/config/paper_config.yaml
# mpiexec -np ${SLURM_NTASKS} $HOME/distributed-HGNNs/build/matrix_bench -t "/cluster/scratch/siwachte" -i ${SLURM_JOB_ID} -p $CPUS
mpiexec -np ${SLURM_NTASKS} $HOME/distributed-HGNNs/build/spmatrix_bench -t "/cluster/scratch/siwachte" -i ${SLURM_JOB_ID} -p $CPUS
