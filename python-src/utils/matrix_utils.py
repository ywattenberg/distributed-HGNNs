...
def simple_dist_mm(A, B, comm: MPI.Comm):
    ...
    if rank == 0:
    # Replace A with your actual CSR matrix
        comm.bcast(A, root=0)
    else:
        A = None
        A = comm.bcast(A, root=0)

    # Scatter the dense matrix to all processes
    local_dense_matrix = np.empty(
        (A.shape[1], B.shape[1] // size), dtype=float
    )
    comm.Scatter(
        [B, MPI.DOUBLE], [local_dense_matrix, MPI.DOUBLE], root=0
    )

    # Perform local multiplication
    print(f"local dens matrix at rank {rank}: {local_dense_matrix}")
    local_result = A.dot(local_dense_matrix)
    print(f"local res at rank {rank}: {local_result}")

    # Gather results from all processes to the root process
    result = None
    if rank == 0:
        result = np.empty(
            (A.shape[0], B.shape[1]), dtype=float
        )
    comm.Gather(
    [local_result, MPI.DOUBLE], [result, MPI.DOUBLE], root=0
)
...
if __name__ == "__main__":
    ...
    res = simple_dist_mm(A, B, comm) 
    
    MPI.Finalize()
    
    if rank == 0:
        print("mpi result: ", res)
        print("gt: ", A @ B)
    
        assert np.all(res ==  A @ B), "results don't match!"