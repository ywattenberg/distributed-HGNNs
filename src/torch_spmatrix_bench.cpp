...
int main(int argc, char* argv[]) {
...

    {
      ...
      for (int scale = 1; scale <= 8; scale*=2) {
        ...
        std::string invde_filename = tmp_dir + "/data/random/scale_" + std::to_string(scale) + "/invDE_HT_DVH.csv";
        torch::Tensor invde_ht_dvh_coo_list = tensor_from_file<float>(invde_filename);
        ...
        for (int i = 0; i < iterations; i++) {
          ...
          auto tmp = invde_ht_dvh.mm(features);
