...
ModelW::ModelW(int in_dim, std::vector<int> lay_dim, int out_dim, double dropout_value, const torch::Tensor *dvh, const torch::Tensor *invde_ht_dvh, bool withBias=false){
    input_dim = in_dim;
    output_dim = out_dim;
    ...
    w = register_parameter("w", torch::ones(dvh->size(1)));
